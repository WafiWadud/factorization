...
def benchmark_primality() -> None:
    ...
    for prime, description in test_primes:
        ...
        result_cached: BenchmarkResult = BenchmarkResult(
            f"{description:30} (cached)", times
        )
        print(result_cached)
...
def benchmark_trial_division() -> None:
    ...
    for n, bound, description in test_cases:
        ...
        result: BenchmarkResult = benchmark(
            trial_division, n, bound=bound, iterations=5
        )
    ...
    if is_simd_available():
        ...
        result_simd: BenchmarkResult = BenchmarkResult(
            "Trial Division SIMD (Numba JIT)", times_simd
        )
        print(result_simd)
...
def benchmark_batch_inverse() -> None:
    ...
    for batch_size in batch_sizes:
        ...
        # Pure Python
        clear_caches()
        result_py: BenchmarkResult = benchmark(
            _batch_inverse_pure_python, values, p, iterations=5
        )
        ...
        if batch_size >= 10:
            result_np: BenchmarkResult = benchmark(
                _batch_inverse_simd, values, p, use_numpy=True, iterations=5
            )
...
def benchmark_ecm_point_ops() -> None:
    ...
    for num_curves in num_curves_list:
        ...
        result: BenchmarkResult = benchmark(
            _ecm_point_double_simd, x_arr, z_arr, a, n, iterations=10
        )
    ...
    for num_curves in [1, 4, 8]:
        ...
        result: BenchmarkResult = benchmark(
            _ecm_scalar_mult_simd, k, P, a, n, iterations=5
        )
...
def benchmark_pollard_rho() -> None:
    ...
    for n, description in test_cases:
        clear_caches()
        result: BenchmarkResult = benchmark(
            pollard_rho_brent, n, use_cache=False, iterations=3
        )
        result.name = description
        print(result)
...
def benchmark_caching_impact() -> None:
    ...
    result_no_cache: BenchmarkResult = BenchmarkResult(
        "Factor (cold cache)", times_no_cache
    )
    ...
    result_cached: BenchmarkResult = BenchmarkResult(
        "Factor (warm cache)", times_cached
    )
    print(result_cached)
    ...
    result_single: BenchmarkResult = BenchmarkResult(
        "Repeated factor() calls (cached)", times_single
    )
    print(result_single)
    first_call: float = times_single[0]
    avg_cached_call: float = statistics.mean(times_single[1:])
    speedup: float = (
        first_call / avg_cached_call if avg_cached_call > 0 else float("inf")
    )
