...
def is_prime(
    n: int, bases: Tuple[int, ...] = (2, 325, 9375, 28178, 450775, 9780504, 1795265022)
) -> bool:
    if n < 2:
        return False
    # small primes check
...
_ecm_attempt_stats: Dict[int, List[Tuple[int, int, int]]] = (
    {}
)  # {n: [(B1, success_count, total_attempts), ...]}
...
def _ecm_point_add(
    P: tuple[int, int], Q: tuple[int, int], diff: tuple[int, int], n: int
) -> tuple[int, int]:
    """
...
def _ecm_impl(
    n: int, B1: int, B2: int | None = None, adaptive: bool = True
) -> int | None:
    """
...
def ecm(
    n: int, B1: int = 100000, B2: int | None = None, use_parallel: bool = False
) -> int | None:
    """
...
def _ecm_attempt(
    n: int, use_parallel: bool = False, try_multiple_B1: bool = True
) -> int | None:
    """
...
def _is_b_smooth(
    val: int, factor_base: list[int], threshold: int = None
) -> tuple[bool, list[int]]:
    """
...
def quadratic_sieve(n: int, use_parallel: bool = False) -> int | None:
    ...
    should_parallelize = (
        use_parallel
        and n > 5 * 10**11  # Only for large numbers (overhead otherwise)
        and _pool is not None  # Pool must exist
    )
    ...
    factor = _structured_gaussian_elimination(n, smooth_values, a_values, factor_base)
    return factor


def _structured_gaussian_elimination(
    n: int, smooth_values: list[list[int]], a_values: list[int], factor_base: list[int]
) -> int | None:
    """
    ...
    return _structured_ge_scipy(n, smooth_values, a_values, factor_base, num_factors)
...
def _structured_ge_scipy(
    ...
    for row_idx in zero_indices:
        factor = _extract_factor_from_dependency_simple(row_idx, a_values, n)
        if factor is not None:
            return factor
...
def _extract_factor_from_dependency_simple(
    zero_row_idx: int, a_values: list[int], n: int
) -> int | None:
    """
...
def _block_lanczos_gf2(
    sparse_matrix: list[list[int]], block_size: int = 32
) -> list[int] | None:
    """
...
def pollard_rho_parallel(n: int) -> int:
    ...
    results: List[int] = pool.starmap(
        _pollard_rho_wrapper, [(n, _) for _ in range(_pool_size)]
    )
...
def factor_internal(n: int, use_parallel: bool = False) -> List[int]:
    ...
    if 10**8 <= rem <= 10**12:
        ...
        if qs_factor is not None:
            return (
                small_factors
                + factor_internal(qs_factor, use_parallel=False)
                + factor_internal(rem // qs_factor, use_parallel=False)
            )
    ...
    if 10**12 <= rem <= 10**18:
        ...
        if ecm_factor is not None:
            return (
                small_factors
                + factor_internal(ecm_factor, use_parallel=False)
                + factor_internal(rem // ecm_factor, use_parallel=False)
            )

    # if still composite, use Pollard Rho (with memoization and optional parallelization)
    use_parallel_rho: bool = (
        use_parallel and rem > 10**12
    )  # Parallel for very large inputs
    ...
    if use_parallel and rem > 10**12:
        ...
        return (
            small_factors
            + factor_internal(d, use_parallel=False)
            + factor_internal(rem // d, use_parallel=False)
        )
